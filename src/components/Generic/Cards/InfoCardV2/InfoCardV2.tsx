  ...
  return (
    <div className="info-card-v2">
      ...
      <div className="info-card-v2_image-wrapper">
        ...
        {data.date && (
          <div className="info-card-v2_date-badge">{data.date}</div>
        )}
      </div>

      {/* Content Section */}
      <div className="info-card-v2_content">
        <h3 className="info-card-v2_title">{data.title}</h3>
        <p className="info-card-v2_description">{data.description}</p>
        <ButtonComponent
          variant="primary"
          label="Read More"
          role="link"
